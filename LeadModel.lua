...
function LeadModel:loss(bb, sp, tgt_in, tgt_out)
    ...
    self:forget()
    local pred_out_table = self:forward(bb, sp, tgt_in)
    local gold_out_table = self.target_out_splitter:forward(tgt_out:t())
    local nll = self.criterion:forward(pred_out_table, gold_out_table)
    local gradLoss = self.criterion:backward(pred_out_table, gold_out_table)
    print(nll)
    self:backward(bb, sp, tgt_in, gradLoss)
...
function LeadModel:greedy_predict(bb, sp)
    ...
    self.tgt_out_step = self.tgt_out_step:resize(32, batch_size)
    ...
    local last_step = 0
    local max_steps = 32 --tgt_in:size(2)
    for step=1,max_steps do
        last_step = step
